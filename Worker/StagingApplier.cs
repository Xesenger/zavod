...
/// the staging manifest. Mismatches are reported but do not abort the apply —
/// the user pressing Accept is authoritative; the mismatch is surfaced as a
/// warning for visibility (they may have edited the file manually between
/// staging and Accept).
/// </summary>
public static class StagingApplier
{
    ...
    public static StagingApplyOutcome Apply(string projectRoot, string taskId)
    {
        ...
        foreach (var result in manifest.Results)
        {
            ...
            if (!projectAbsolute.StartsWith(normalizedRoot, StringComparison.OrdinalIgnoreCase))
            {
            ...
            if (!string.IsNullOrEmpty(result.OriginalSha256) && File.Exists(projectAbsolute))
            {
                try
                {
                    ...
                    {
                        hashWarnings.Add($"{result.Path}: project file changed since staging (staged-origin sha256={result.OriginalSha256[..8]}…, current sha256={currentHash[..8]}…). Apply proceeded; review the edit.");
                    }
                }
                catch (IOException ex)
                {
                    hashWarnings.Add($"{result.Path}: could not re-read project file for hash guard ({ex.Message}). Apply proceeded.");
                }
            }
            ...
            try
            {
                ...
                var stagedContent = File.ReadAllText(result.StagedAbsolutePath, Encoding.UTF8);
