...
> This document defines the intended architecture for the verification layer.
> `ExecutionVerificationResult` and the three-layer pipeline do not exist in code yet.
> The boundary position (Worker → Verification → Result UI) is correct and must be preserved when implemented.
